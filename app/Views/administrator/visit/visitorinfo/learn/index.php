        ...
        <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            <div class="d-flex align-items-center flex-wrap mr-1">
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    <h5 class="text-dark font-weight-bold my-1 mr-5">Visitor Info – Learn Section</h5>
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        <li class="breadcrumb-item text-muted"><a href="#" class="text-muted">Visit</a></li>
                        <li class="breadcrumb-item text-muted"><a href="#" class="text-muted">Learn Section</a></li>
                    </ul>
                </div>
            </div>
        </div>
            ...
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-3">
                    <div class="card-title">
                        <h3 class="card-label">Section: <em>Edukasi &amp; Konservasi</em> <span class="d-block text-muted pt-2 font-size-sm">Konten kartu edukasi untuk .vi-learn-section</span></h3>
                    </div>
                    <div class="card-toolbar">
                        <a href="<?= base_url('adminsite/visit/visitorinfo/learn/add') ?>" class="btn btn-primary font-weight-bolder">+ Tambah Item</a>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <tbody>
                            <?php $i = 1; foreach ($getdata as $rs): ?>
                                <tr>
                                    <td><?= $i++ ?></td>
                                    <td>
                                        <?php if (!empty($rs['visitorinfo_image'])): ?>
                                            <img src="<?= base_url('assets/upload/visitorinfo/' . esc($rs['visitorinfo_image'])) ?>" alt="image" style="max-height:50px; border-radius:4px;">
                                        <?php else: ?>–<?php endif; ?>
                                    </td>
                                    <td>
                                        <span class="label label-pill label-inline label-light-primary"><?= esc($rs['visitorinfo_label'] ?? '–') ?></span>
                                        <?php if (!empty($rs['visitorinfo_label_en'])): ?>
                                            <span class="label label-pill label-inline label-light-info ml-1"><?= esc($rs['visitorinfo_label_en']) ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?= esc($rs['visitorinfo_title']) ?></td>
                                    <td><?= esc($rs['visitorinfo_title_en'] ?? '') ?></td>
                                    <td><?= esc((string) ($rs['visitorinfo_sort'] ?? 0)) ?></td>
                                    <td>
