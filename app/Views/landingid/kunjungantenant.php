...
$normalizeTenantText = static function (?string $value, ?string $title = null, int $limit = 0): string {
  $text = bxsea_plain_text($value);
  $text = str_ireplace('souvernir', 'souvenir', $text);
  if ($title && stripos($title, 'BXSea Merchandise Store') !== false) {
    $text = 'Jadikan momen petualanganmu di BXSea semakin berkesan dengan koleksi merchandise resmi kami yang bisa dibawa pulang sebagai kenang-kenangan spesial.';
  }
      ...
      <div class="row descBanner">
        <div class="col-lg-12 box-schoolpackage">
          <div class="desc-schoolpackage">
            <h1>TENANT KAMI</h1>
            <p>Nikmati berbagai pilihan kuliner dan layanan eksklusif di BXSea.</p>
          </div>
        </div>
      </div>
    </div>
